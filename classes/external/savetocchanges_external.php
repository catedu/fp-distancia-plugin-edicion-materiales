 ...
 * @category   external
 */
class savetocchanges_external extends external_api {

    /**
     * @return external_function_parameters
     */
    public static function savetocchanges_parameters(): external_function_parameters {
        return new external_function_parameters(
            [
                ...
                'html' => new external_value(PARAM_RAW, 'HTML for include in file'),
                'other' => new external_value(PARAM_RAW, 'HTML for include in file', VALUE_OPTIONAL)
            ]
        );
    }
     ...
     */
    public static function savetocchanges(int $courseid, int $resourceid, string $versionname, string $html, string $other = ''): array {
        global $DB, $PAGE;
        self::validate_parameters(
            self::savetocchanges_parameters(),
            [
                'courseid' => $courseid,
        ...
        return [
            'response' => $manageeditable->savetocchanges($html, $other),
            'tochtml' => @$manageeditable->get_toc_for_edit()
        ];
    }
     ...
     * @return external_function_parameters
     */
    public static function savetocchanges_returns(): external_function_parameters {
        return new external_function_parameters(
            [
                'response' => new external_value(PARAM_BOOL, 'response', VALUE_REQUIRED),
                'tochtml' => new external_value(PARAM_RAW, 'response', VALUE_REQUIRED),
            ]
        );
    }
}
